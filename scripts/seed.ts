const { categories, accounts, transactions } = require("@/db/schema");
...
const generateRandomAmount = (category: typeof categories.$inferInsert) => {
 ...
 const generateTransactionsForDay = (day: Date) => {
    ...
    for (let i = 0; i < numTransactions; i++) {
        const category = SEED_CATEGORIES[Math.floor(Math.random() *
    SEED_CATEGORIES.length)];
    const isExpense = Math.random() > 0.6;

    const amount = generateRandomAmount(category);
    const formattedAmount = convertAmountToMiliunits(isExpense ?
        -amount : amount);
    ...
    SEED_TRANSACTIONS.push({
        ...
        payee: "Merchant",
        notes: "Rendom transaction",
    });
   }
 };

 const generateTransactions = () => {
    const days = eachDayOfInterval({ start: defaultFrom, end:
    defaultTo });
